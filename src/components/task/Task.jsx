...
export const Task = ({ task, taskList, setTaskList }) => {
  ...
  const editTask = (id) => {
    ...
    const newTaskList = taskList.map((task) => {
      if (task.id == id) {
        //Edito lo que esta en el Local Storage (No es lo del filtro)
        localStorageTaskList.map((t) => {
          if (t.title == task.title) {
            t.title = inputEdit;
          }
          return t;
        });
        //Ahora edito lo que está en los filtros
        task.title = inputEdit;
      }
      return task;
    });
    setTaskList(newTaskList);
    setLocalStorage("taskListStorage", localStorageTaskList);
    toast({
      title: "Tarea editada.",
  ...
  return (
    ...
    >
      <Text as={task.done ? "del" : ""} fontSize="xl" noOfLines={2}>
        {task.title}
      </Text>
      <HStack>
        <IconButton
